...
있었습니다 — 주최(host) 정보에 붙는 `type` 필드였어요. 처음엔 명세서 부록과 필드표가
서로 달라서(부록은 "제거 예정", 필드표는 "포함") 일단 필드표 기준으로 남겨뒀는데,
이후 주최 상세 화면을 만들면서 "확정된 ERD에 없는 필드"로 결론이 나 화면 쪽은 처음부터
이 필드를 참조하지 않았습니다. 그 결론이 목데이터 응답·프론트 타입 선언에는 한동안
반영이 안 된 채로 남아 있었는데(#64), 실제로 지워보니 걸린 범위가 예상보다 넓었습니다 —
...
전부 지우고서야 `tsc`가 깨끗해졌습니다. 지금은 목데이터·타입 선언 어디에도 이 필드가
없습니다 — 실제 API도 이 필드를 안 내려주는 것으로 맞춥니다.
