...
네, 실제로 있습니다 — 주최(host) 정보에 붙는 `type` 필드예요. 처음엔 명세서 부록과
필드표가 서로 달라서(부록은 "제거 예정", 필드표는 "포함") 일단 필드표 기준으로
남겨뒀는데, 이후 주최 상세 화면을 만들면서 "확정된 ERD에 없는 필드"로 결론이 나 화면
쪽은 처음부터 이 필드를 참조하지 않습니다. 그 결론이 목데이터 응답·프론트 타입
선언에는 아직 반영이 안 된 채로 남아 있어요(#64) — 화면이 안 쓰니 동작엔 영향 없지만,
결론 난 항목이 코드에는 "미확정"으로 남아 있는 상태입니다.

실제로 지우려면 범위가 처음 추정(`hosts.ts` 한 줄)보다 넓습니다 — 확인해보니
`hosts.ts` 응답, `festivals.ts`의 host 요약 3곳(목록 카드·축제 상세·다가오는 축제),
`search.ts`가 `hostType`이라는 다른 이름으로 한 번 더 내려주던 같은 필드, 프론트 타입
선언 3곳(`HostSummary`/`FestivalHostSummary`/`HostResult`), showcase 표본 데이터까지
`tsc`로 잡히는 참조가 8개 파일에 걸쳐 있었습니다. 필드 제거 자체는 문서 정리와
별개의 코드 변경이라 이슈를 따로 파서 진행합니다.

## 4. 한계 (솔직하게)

- 분실물·관리자 화면은 아직 목데이터가 없습니다.
