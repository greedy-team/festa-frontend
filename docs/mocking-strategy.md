...
컴포넌트는 평소처럼 `fetch()`를 부르고, 그 요청을 MSW가 네트워크 레벨에서 가로채 응답을
대신 돌려줍니다. 컴포넌트 입장에서는 지금 목데이터를 받는지 실제 API를 받는지 알 방법이
...
네, 실제로 있습니다 — 주최(host) 정보에 붙는 `type` 필드예요. `hosts.ts` 응답뿐 아니라
`festivals.ts`가 내려주는 host 요약 3곳(목록 카드·축제 상세·다가오는 축제)에도 같은
필드가 있습니다. 처음엔 명세서 부록과 필드표가 서로 달라서(부록은 "제거 예정", 필드표는
"포함") 일단 필드표 기준으로 남겨뒀는데, 이후 주최 상세 화면을 만들면서 "확정된 ERD에
없는 필드"로 결론이 나 화면 쪽은 이미 이 필드를 참조하지 않습니다. 목데이터 응답·프론트
타입 선언에서는 아직 안 지웠어요 — 화면이 안 쓰니 동작엔 영향이 없지만, "결론 난 걸
목데이터 쪽에 반영 안 한 채로 둔" 사례라 정리 대상으로 남아 있습니다.
