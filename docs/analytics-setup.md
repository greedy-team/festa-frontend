...
- Clarity Strict 마스킹을 설정한다. 코드에서도 body 전체를 `data-clarity-mask`로 마스킹한다.
...
Clarity는 압축 해제한 JSON에 검사 대상 DOM 노드가 포함됐음을 확인하고 본문·href·title·
aria-label·검색 입력의 테스트 문자열이 빠졌는지 검사했다. 이 회귀 검사를 포함한 E2E 7개가
