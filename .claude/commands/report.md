...
화면 캡처·테스트 리포트처럼 **나중에 다시 만들 수 없는 그 시점의 결과물**만 저장소에 남깁니다.

- 위치는 `docs/reports/assets/<이슈번호>/`. 보고서에서 상대경로(`./assets/<번호>/<파일>`)로 링크하고,
  화면 캡처는 인라인 이미지(`![설명](경로)`)까지 답니다
- `test-results/`·`playwright-report/`는 **gitignore 그대로 둡니다** — 매 실행마다 통째로 바뀝니다.
  남길 것이 있으면 그 시점의 파일만 골라 assets로 복사합니다
- 파일명은 무엇을 담았는지 알아보게 짓습니다 (`admin-festivals.png`, `festival-detail-map-375.png`).
  뷰포트가 쟁점이면 폭을 접미어로 붙입니다
- **남길 게 없으면 절을 만들지 않습니다.** 회귀 확인만 한 실행의 리포트 HTML은 보존 가치가 없습니다
...
#### route.ts
`request.formData()`로 파일을 받아 `image-upload.ts`의 업로드 함수에 전달
