...
- **연도는 여러 해가 섞이는 목록 카드에만 얹는다** — 축제 전체 목록과 홈 "최근 등록된 축제" 두 곳. 기간 바로 위 별도 줄이고, 해를 걸치는 축제는 `2025 ~ 2026`으로 두 연도를 적는다 (#182 / DEC-0170).
...
- **히어로·상세·검색 결과 행에 연도를 넣지 않는다.** 그 화면들은 이미 하나의 축제를 보고 있거나 검색 조건이 걸려 있어 연도가 중복 정보다. 홈 히어로는 D-day가 시간을 말하는 자리라(DEC-0059) 절대 날짜를 키우는 방향과도 어긋난다.
