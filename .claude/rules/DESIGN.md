...
- 전체: day-card의 `+더보기`나 섹션 헤더의 "전체 라인업 보기"를 누르면 바텀시트가 뜬다.
  DAY 탭(전체 포함)으로 필터하고, 각 아티스트를 번호·아바타·이름으로 나열한다. 헤드라이너
...
"히어로 클릭 인터랙션" 참고), 공지·교통편 안내, 분실물 연동, 광고 슬롯(홈 외 화면의 광고
정책이 아직 없다), 과거 라인업(같은 축제명의 지난 회차 아카이브 — 시안엔 있지만 이 API 응답에
대응 데이터가 없어 임의로 만들지 않았다. 필요해지면 백엔드 스펙과 함께 다시 다룬다).
