...
- 홈에서 축제를 눌렀을 때 이 시트가 뜬다 (`06-D-1 Home D-day (요약 시트)`, node `683:482`). **이 시트는 현행 4분할 히어로 위에 그려져 있어 06-D와 일치한다.**
...
### 미결 결정
- **히어로 카드 클릭 인터랙션이 미정이다.** 캔버스 메모(`734:734`)에 세 안이 있다 — 명준: 클릭한 카드가 왼쪽으로 이동하고 오른쪽에 설명, 규형: 상단에서 커튼콜처럼 내려옴, 의민: 상세 화면으로 이동(임시). 현재 구현 기준은 의민 안(06-D-1 요약 시트)이고, 명준 안 기준 레이아웃 3안(인라인 확장 / 히어로 스플릿 / 포스터 리프트)까지 검토됐으나 확정되지 않았다.
