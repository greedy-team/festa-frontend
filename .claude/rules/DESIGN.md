...
로고가 없거나 로드에 실패해도 깨진 이미지 대신 틴트가 남는 것을 정상 경로로 둔다. 주최 공식
사이트(`homepageUrl`)가 있으면 축제 상세 히어로와 같은 우상단 원형 아이콘 링크로 얹는다 —
없으면 자리를 그리지 않는다. 인스타그램은 이 응답 계약에 없어 자리를 만들지 않는다(DEC-0107).
...
day-card·라인업 바텀시트, 주최 상세(#46)의 자주 온 아티스트 카드가 아직
`gridTint(artistId)`로 이니셜 없는 색 원/사각형만 쓰고 있다 — 이 PR보다 먼저 만들어져서
가져다 쓸 수 없었다. develop에 머지된 뒤 그 자리들을 `artist-avatar`로 교체하는 후속
작업이 필요하다. 아티스트 목록(#49)의 카드와 검색 결과 카드(#53)는 이 PR에서 같이
교체했다.

**섹션 순서**: 히어로 → 예정 공연/출연 이력(좌우 2단, 넓은 쪽이 예정 공연). 사진이 없어
텍스트 중심 화면이 된다 — 아바타의 색·이니셜이 유일한 시각적 구분 요소다.

**히어로** — 아바타(정사각) 옆에 장르 칩 → 이름 → 다른 이름(있으면) 순으로 쌓는다.
인스타그램이 있으면 아이콘을 이름 옆이 아니라 아바타 모서리에 배지로 겹친다 — "이 아바타의
계정"이라는 연결이 이름 옆보다 분명하다. **장르가 없으면 칩 자리를 비워두지 않고 아예
