...
- 요약: `{component.day-card}`를 day 수만큼 가로로 늘어놓는다(좁은 화면에서는 세로로 쌓는다).
  하루당 아티스트를 앞에서 3명만 이름으로 세로 나열하고, 4명이 넘으면 카드 안에
  `+ 더보기`를 둔다. 아바타 자리는 두지 않는다 — 실사진은 초상권 때문에 쓰지 않고
  (DEC-0063), 빈 자리를 색 블록으로 채우지도 않는다(DEC-0130). 시크릿 게스트(`id`가
  null인 원소 — DEC-0116, 응답에 `revealed` 불리언을 두지 않고 `id` null 여부로만
  판별한다)는 이름 대신 "공개 예정"을 `{colors.muted-soft}`로 낮춰서 자리를 그대로
  보여준다 — 자리를 비우면 "아직 공개 안 됨"과 "원래 라인업이 작음"이 구분되지 않는다.
- 전체: day-card의 `+더보기`나 섹션 헤더의 "전체 라인업 보기"를 누르면 바텀시트가 뜬다.
  DAY 탭(전체 포함)으로 필터하고, 각 아티스트를 번호·이름·장르로 나열한다. 아바타
  자리는 두지 않는다(위와 같은 이유). 장르가 없으면 자리를 그리지 않는다 — 아티스트
  상세 화면과 같은 관례. 헤드라이너 표시는 넣지 않는다 — 명세에 그런 필드가 없어서
  만들 근거가 없다. 생기면 그때 얹는다.
...
**`day-card`** (410×200) — DAY 뱃지 + 날짜 13/400, 이름 최대 3개를 17/700로 세로 나열(공개
예정은 `{colors.muted-soft}`), 하단 `+ 더보기` 13/500 인디고. 아바타 자리는 두지 않는다.
