...
| 패널 | 틴트 | 예시 |
|---|---|---|
| 1 | `{poster-tints.hero-1}` #315F7D | 연세대 아카라카 · D-2 |
...
**스크림** — 패널마다 하단 `360×364` 순검정 55% (`{colors.scrim-hero}`). 단색 오버레이이며 그라데이션이 아니다. 패널 y660에서 시작해 y1024에서 패널과 함께 끝난다. **구현은 이 띠를 쓰지 않는다** — 아래 "개수별 조판" 참고. 시안은 아직 띠 기준이라 피그마 반영이 필요하다([반영 대기](#반영-대기)).
...
**개수별 조판 (#187)** — 위 좌표는 축제 4건·패널 360 기준이다. 축제가 그보다 적으면 패널이
1/2·1/3·전폭으로 넓어지는데(Hero.tsx `slideBasisClass`), 그때 내용을 좌하단에 그대로 두면
화면 대부분이 빈 포스터 면만 남는다 — 실데이터가 1건인 지금 그것이 홈의 기본 화면이었다.
조판은 **개수로만** 갈리고 화면 폭은 CSS 브레이크포인트가 받는다(JS로 폭을 재지 않는다,
LSN-0020):

| 축제 수 | 패널 | 조판 |
|---|---|---|
| 3건 이상 | ≤ 480 | **세로 스택** — D-day(y140) → 포스터(남는 높이 가운데, 원본 비율) → 정보(하단 56) |
| 2건 | 720 | `xl`(1280)부터 **좌우 분할** — 왼쪽 텍스트 열(D-day → 학교 → 축제명 30 → 날짜·장소 16 → 흰 CTA) \| 오른쪽 포스터(패널 높이 62%). 그 아래 폭은 스택 |
| 1건 | 1440 | `lg`(1024)부터 **좌우 분할** — 축제명만 `{typography.hero}` 48로 키우고 인셋 80, 포스터는 패널 높이 78%. 그 아래 폭은 스택 |

- 포스터는 어느 조판에서도 **자르지 않는다** — 원본 비율로 넣고, 남는 자리는 같은 포스터를
  흐리게 깔아(blur) 그 축제의 색으로 메운다. 새 틴트를 만들지 않는다.
- 스크림 띠(하단 364, 55%)는 쓰지 않는다 — 텍스트가 포스터 옆·아래로 빠져 띠가 받칠 자리가
  없고, 띠가 포스터 하단(날짜·장소가 인쇄된 부분)을 가렸다. 대신 **패널 전면 순검정 35%** 한 장 +
  흰 글자 그림자(`0 2px 16px` 45%)로 대비를 낸다. 히어로에 인디고를 넣지 않고 흰색 + 불투명도
  100/85/75로 위계를 내는 규칙은 그대로다.
- CTA(흰 채움 48 + ink `자세히 보기 →`)는 좌우 분할에서만 그린다. 스택에서는 칸 전체가
  링크라 따로 두지 않는다.
- 분기점이 개수마다 다른 이유: 1건은 패널이 전폭이라 1024에서 이미 텍스트 열 옆에 포스터가
  서지만, 2건은 반폭이라 패널이 640은 돼야(1280) 같은 구성이 들어간다. 3건은 lg에서도
  341이라 나눌 폭이 없다.
...
- **연도** `{typography.label-regular}` 12/400 `{colors.muted-soft}` — y좌표 없음 (#182)
- 기간 12/400 `{colors.muted-soft}` — y1504 (카드 하단 y1519)

포스터 위에 상태 도트·D-day 뱃지를 얹지 않는다. **홈의 D-day는 히어로에만 있다.**

> **연도 줄만 y좌표가 없다.** 위 좌표와 카드 하단은 전부 연도 줄이 없던 시점의 시안
> 실측값이고, 구현이 네 줄로 앞서 있다. 치수는 피그마 실측값이 원본이므로
> ([Iteration Guide](#iteration-guide) 8) 코드에서 잰 높이를 여기 적지 않는다 —
> [반영 대기](#반영-대기) 참고. 연도와 기간은 같은 크기·색을 쓰고 간격만 좁혀
> (8px → 4px) 한 덩어리로 읽히게 한다.
...
기준: 08/08-2 Festival Detail 시안. 08-3(전체 라인업 바텀시트)은 쓰지 않는다 — 아래 라인업 절
참고(#168). 이 화면부터는 1440 좌표를
...
**히어로** — `{component.upcoming-host-card}`와 같은 포스터 전면형이되, 스크림은 홈 히어로와
같은 `{colors.scrim-hero}`(순검정 55%) 한 단계다(그라데이션 아님). ink 3단은 쓰지 않는다 —
밝은 포스터(세종연회 라벤더) 위에서 흰 글자 대비가 `scrim-35`는 2.5:1로 AA(4.5:1)에 못 미치고
검정 55%는 5.2:1이다(#165). 축제명 아래 날짜·장소·티켓 3줄은 `{typography.body}` 16/500 흰
100%, 아이콘 16. 포스터가 없거나 로드에 실패하면 포스터 틴트만 남는 것을 정상 경로로 둔다(홈과 동일한
...
축제 공식 인스타그램(응답 최상위 `instagramUrl`)·주최 공식 사이트(`host.homepageUrl`)가 있으면
우상단에 아이콘 링크로 얹는다(둘 다 없으면 자리를 그리지 않는다). 인스타는 학교 공식 계정
(`host.instagramUrl`)이 아니라 축제 계정을 쓴다 — 축제 상세는 그 축제 하나를 보는 화면이다
(#190). 학교 계정은 응답에 그대로 오지만 이 화면에서는 노출하지 않는다.

**라인업** — 한 층이다. `{component.day-card}`를 day 수만큼 가로로 늘어놓고(좁은 화면에서는
세로로 쌓는다) 카드가 그 날의 아티스트를 **전원** 세로 나열한다. 바텀시트·"전체 라인업 보기"·
`+ 더보기`는 두지 않는다 — 실데이터가 하루 4~5팀이라 3명에서 끊고 시트로 보내던 두 층 구조가
할 일이 없었다(#168). 하루 열 팀을 넘는 축제가 들어오면 시트가 아니라 카드 안 접기로 대응한다.
- 이름 17/700(`{typography.entity-name}`), 장르는 이름 옆에 `{typography.caption}` 14/400
  `{colors.muted}` — 장르가 없으면 그리지 않는다(아티스트 상세와 같은 관례). 순번은 배열 순서가
  곧 순번이라(DEC-0109) 따로 적지 않는다.
- 공개된 아티스트 행은 **아티스트 상세로 가는 링크**다. 행 전체가 타깃이고 우측에 `ChevronRight`
  16 `{colors.muted-soft}`, hover는 `{colors.surface-field}` 배경 — 아티스트 목록 행(`ArtistRow`)·검색
  결과 행과 같은 문법이다. hover가 없는 모바일에서는 chevron이 유일한 어포던스라 뺄 수 없다.
  "공개 예정" 행은 링크가 아니며 chevron도 hover도 없다.
- 아바타 자리는 두지 않는다 — 실사진은 초상권 때문에 쓰지 않고(DEC-0063), 빈 자리를 색
  블록으로 채우지도 않는다(DEC-0130). 시크릿 게스트(`id`가 null인 원소 — DEC-0116, 응답에
  `revealed` 불리언을 두지 않고 `id` null 여부로만 판별한다)는 이름 대신 "공개 예정"을
  `{colors.muted-soft}`로 낮춰서 자리를 그대로 보여준다 — 자리를 비우면 "아직 공개 안 됨"과
  "원래 라인업이 작음"이 구분되지 않는다.
- 헤드라이너 표시는 넣지 않는다 — 명세에 그런 필드가 없어서 만들 근거가 없다. 생기면 그때
  얹는다.
...
홈은 236px 카드가 5개 늘어서는 고밀도 화면이라 12~14px이 맞지만, 상세 화면(축제·주최·아티스트)은
요소 폭이 넓고 담는 정보가 적어 같은 크기가 상대적으로 더 작아 보인다. 상세 화면에서는:

1. **읽어야 하는 텍스트는 `{typography.caption}`(14) 아래로 내려가지 않는다.** `meta`(13)·`label`(12)은
   순번·뱃지·보조 표시 전용이다.
2. **`{colors.muted-soft}`는 읽지 않아도 되는 자리에만 쓴다** — 순번, placeholder, 비활성("공개 예정"),
   광고 라벨. 흰 배경 대비 2.5:1로 AA(4.5:1)에 못 미친다. 읽어야 하는 텍스트는 `{colors.muted}`(4.8:1)
   이상.
3. **라벨·값 쌍은 라벨의 대비를 값과 같은 단계 이상으로 두고, 크기는 값이 한 단계 위.** 라벨이 값보다
   흐리면 표로 읽히지 않는다.

주최 상세·아티스트 상세에도 같은 규칙을 적용했다(#165) — 브레드크럼·"전체 N개 →"·카드의
시즌/기간·출연 이력의 연도/학교·기간이 `caption` 14 `{colors.muted}`로 올라갔다. 검색 결과 행은
이미 `caption-strong` `{colors.muted}` 메타 한 줄이라 코드는 그대로이고 `{component.result-row}`
스펙의 라벨 색만 `muted`로 맞췄다. 순위·순번(`{colors.muted-soft}`)은 장식이라 그대로다.
...
**자주 온 아티스트** — 순위(`01`~`03`) + 이름 + 출연 횟수 3장을 나열한다. 아바타 자리는
두지 않는다 — 실사진은 초상권 때문에 쓰지 않고(DEC-0063), 빈 자리를 색 블록으로
채우지도 않는다(DEC-0130).
...
| 히어로 | 최근 축제 |
|---|---|
| #315F7D · #3B1F2E · #1F3A2E · #2A2340 | #2F4F6E · #3A1F6B · #3E5F52 · #6B2B3A · #374151 |
...
### Scrim
두 계열이 공존한다.
- **히어로** — 순검정 단일 단계. `{colors.scrim-hero}` 55%, 패널 하단 364px. 그라데이션 아님. 축제 상세 히어로도 같은 토큰을 전면에 쓴다(#165).
...
**`recent-card`** (236×379 — 시안 실측, 연도 줄 추가 전) — 포스터 `236×300` `{rounded.media}` 18 + 학교명 12/400 `{colors.muted}` + 축제명 17/700 `{colors.ink}` + **연도 12/400 `{colors.muted-soft}`** + 기간 12/400 `{colors.muted-soft}`. 오버레이 없음. 연도는 #182에서 들어왔고 시안에는 아직 없다 — [반영 대기](#반영-대기).
...
**`festival-card`** (236×400 — 시안 실측, 연도 줄 추가 전) — 포스터 236×320 r18 + 좌상단 상태 도트, 아래 학교명 12/400 → 축제명 17/700 → **연도 12/400 `{colors.muted-soft}`** → 기간 12/400. 연도는 #182에서 들어왔고 시안에는 아직 없다 — [반영 대기](#반영-대기).
...
**`day-card`** (410×가변) — DAY 뱃지 + 날짜 `{typography.caption}` 14/400 `{colors.muted}`, 그 날의
아티스트 전원을 17/700로 세로 나열(공개 예정은 `{colors.muted-soft}`), 장르는 이름 옆 14/400
`{colors.muted}`, 행 전체가 아티스트 상세 링크(우측 chevron 16 `{colors.muted-soft}`, hover `{colors.surface-field}`). `+ 더보기`·바텀시트 없음(#168). 아바타 자리는 두지 않는다.

### List Rows

**`result-row`** (1280×148) — 100×100 썸네일 r12, 제목 20/600, 라벨/값 2열 그리드 3세트 (라벨 14/400 `{colors.muted}` / 값 14/500 `{colors.ink}` — 상세 화면 공통 텍스트 규칙 ②), 우측 `자세히 보기 →`.
...
- **여러 해가 섞이는 목록 카드는 연도를 기간 위 별도 줄로 얹는다** — 축제 전체 목록과 홈 "최근 등록된 축제" 두 곳. `festivalYear`, 해를 걸치는 축제는 `2025 ~ 2026` (#182 / DEC-0170, DEC-0178로 승계).
- **하나의 축제를 봐도 몇 년인지 알아야 하는 화면은 날짜 줄 자체에 연도를 넣는다** — 축제 상세 히어로, 검색 결과 축제 행, 관리자 검수 표(기간 열 + 모바일 서브라인). 줄을 늘리지 않고 `dateRangeWithYear`로 `2026.05.07 ~ 2026.05.09`, 해를 걸치면 `2025.12.30 ~ 2026.01.02` (#202 / DEC-0178).
- **공용 `dateRange`(`05.07 ~ 05.09`)는 바꾸지 않는다** — 연도 없는 화면(홈 히어로·일자별 라인업 카드·주최 상세 다가오는 축제 카드)이 함께 쓰므로.
...
- **홈 히어로·일자별 라인업 카드·주최 상세 다가오는 축제 카드에는 연도를 넣지 않는다.** 홈 히어로는 D-day가 시간을 말하는 자리라(DEC-0059) 날짜를 키우는 방향과 어긋나고, 일자별 라인업 카드는 같은 화면의 히어로가 연도를 이미 말하며, 다가오는 축제 카드는 D-day 배지가 붙어 있다 (#202 / DEC-0178).
...
- **목록·최근 카드에 연도 줄이 들어가 실제 높이가 커졌는데 피그마는 세 줄 그대로다.** `{component.recent-card}`(236×379)와 `{component.festival-card}`(236×400), 홈 구조의 y1456·y1476·y1504·카드 하단 y1519는 전부 **연도 줄 추가 전** 시안 실측값이다. 코드에서 잰 높이를 문서에 넣지 않는 이유는 Iteration Guide 8(추정값 금지, 피그마 실측만) 때문이다. **피그마에 연도 줄을 반영한 뒤 네 값을 함께 갱신해야 한다.**
- **히어로 패널 조판이 개수별로 갈리는데 피그마는 4건·스크림 띠 한 가지뿐이다.** 구현은 1건(좌우 분할, 축제명 48)·2건(좌우 분할)·3건 이상(세로 스택)으로 나뉘고 스크림 띠 대신 전면 35%를 쓴다(#187, [개수별 조판](#hero-4분할-풀블리드-캐러셀) 참고). **1·2·3건 프레임과 스크림 변경을 피그마에 그려야 한다.**
