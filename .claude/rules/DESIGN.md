...
- 요약: `{component.day-card}`를 day 수만큼 가로로 늘어놓는다(좁은 화면에서는 세로로 쌓는다).
  ...
  `+ 더보기`를 둔다. 미공개(`revealed: false`) 아티스트는 원형을 `{colors.divider}`로,
...
**히어로** — 배너 이미지(없으면 포스터 틴트) 위에 로고(원형)·약칭·지역·이름을 얹는다. 배너·
로고가 없거나 로드에 실패해도 깨진 이미지 대신 틴트가 남는 것을 정상 경로로 둔다.
...
**`artist-avatar` (신규, 공용 컴포넌트로 확정) — 이니셜 + 이름 해시 색상.** DEC-0063(초상권
문제로 아티스트 실사진을 쓰지 않는다)에 대한 답이 이 이슈에서 확정됐다. 포스터 틴트가 쓰는
"식별자를 해시해 팔레트에서 색을 고른다"는 원리를 이름 문자열에 그대로 적용하고, 그 위에
이름 첫 글자를 얹는다. 같은 이름은 항상 같은 색 — 순수 함수라 새로고침해도 안 바뀐다.
자리에 따라 두 모양을 쓴다: 목록·카드처럼 좁은 자리는 원형(`shape="circle"`), 상세 화면
히어로처럼 큰 자리는 정사각형(`shape="square"`, festival-card 포스터와 같은 r18).

**⚠️ 반영 대기 — 이 컴포넌트가 생기기 전에 이미 만들어진 화면들.** 축제 상세(#45)의
day-card·라인업 바텀시트, 주최 상세(#46)의 자주 온 아티스트 카드, 아티스트 목록(#49)의
카드가 전부 `gridTint(artistId)`로 이니셜 없는 색 원/사각형만 쓰고 있다. 세 PR 모두 이
PR보다 먼저 만들어져서 이 컴포넌트를 가져다 쓸 수 없었다. 전부 develop에 머지된 뒤, 그
자리들을 `artist-avatar`로 교체하는 후속 작업이 필요하다.

**섹션 순서**: 히어로 → 예정 공연 → 출연 이력. 사진이 없어 텍스트 중심 화면이 된다 — 아바타의
색·이니셜이 유일한 시각적 구분 요소다.

**히어로** — 아바타(정사각) 옆에 장르 칩 → 이름 + 외부 링크 아이콘 → 다른 이름(있으면) 순으로
쌓는다. **장르가 없으면 칩 자리를 비워두지 않고 아예 그리지 않는다** — 실측 다수(133명 중
36명)가 빈 값이라 "미분류" 같은 대체 문구를 새로 만들지 않는다.

**예정 공연 / 출연 이력** — 둘 다 응답이 5건 + 전체 건수만 준다. 페이지네이션도 "더 보기"
링크도 만들지 않는다 — 전체 목록은 축제 목록 화면(`artistId` 필터)의 몫이다. **예정 공연의
D-day는 서버가 이미 공연일(축제 시작일이 아니라) 기준으로 계산해 내려준다** — 다시 계산하지
않고 포맷만 한다. 출연 이력은 이 응답 구조(1건 = 1행)에 맞춰 연도·축제명·학교·기간을 한 행에
쌓는 전용 행을 쓴다 — DESIGN.md의 `{component.past-lineup-row}`(연도 하나에 아티스트 여러
명을 나열하는 구조)는 반대 방향 데이터라 재사용하지 않는다.

**뺀 것** — 좋아요·알림(로그인 없음), 아티스트 사진(초상권), 광고 슬롯.
