 ...
 *
 * 경로 접두사 `/api`는 `API_BASE`가 붙인다(`lib/api.ts`, #127). 호출부는
 * `/admin/...`만 넘기고 여기서도 다시 붙이지 않는다 — 백엔드가 이 접두사를 두 번
 * 뒤집었으므로(PR #28 제거 → PR #62 복원, DEC-0084 폐기) 고칠 곳을 한 곳으로 모은다.
 *
...
export async function adminFetch<T>(
  ...
  // 204 No Content에는 파싱할 본문이 없다 — DELETE 계열이 전부 여기로 온다.
  // 이 가드가 없으면 성공한 삭제가 응답 파싱 실패로 잡혀 UNKNOWN 에러가 된다.
