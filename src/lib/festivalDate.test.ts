...
describe("dDay", () => {
  ...
  // #218: 진행 중인 축제는 날짜가 며칠 남았든 D-DAY로 고정한다 (D+N도, 종료일까지
  // 남은 일수도 아니다 — "오늘이 축제 날인가"만 알려준다).
  it("시작일 당일이면 D-DAY", () => {
    expect(dDay("2026-09-09", "2026-09-11", "2026-09-09")).toBe("D-DAY");
  });

  it("진행 중 가운데 날짜도 D-DAY", () => {
    expect(dDay("2026-09-09", "2026-09-11", "2026-09-10")).toBe("D-DAY");
  });
