...
type Tone = "primary" | "success" | "on-media";
...
export function Badge({ tone = "primary", className = "", children }: Props) {
  return (
    // px-3(12px)는 DESIGN.md에 없는 기본값이다. 시안은 높이만 28로 고정하고
    // 폭은 내용에 맡긴다. badge-day(64×28)는 className으로 폭을 준다.
    <span
      className={`inline-flex h-[28px] items-center justify-center rounded-pill px-3 ${TONE[tone]} ${className}`}
    >
      {children}
    </span>
  );
