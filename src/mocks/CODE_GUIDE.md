...
- `type` 필드는 변경사항 메모(제거 예정)와 실제 필드표(포함)가 서로 달라 일단 필드표 기준으로 포함해뒀다. 이후 주최 상세 화면(#46) 작업에서 "확정된 ERD에 없는 필드"로 결론 나서 화면 쪽은 이미 이 필드를 안 쓴다 — 다만 이 mock 응답에서는 아직 지우지 않았다(안 써도 동작엔 영향 없음).
