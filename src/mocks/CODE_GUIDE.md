...
- `type` 필드는 변경사항 메모(제거 예정)와 실제 필드표(포함)가 서로 달라 일단 필드표 기준으로 포함해뒀다. 이후 주최 상세 화면(#46) 작업에서 "확정된 ERD에 없는 필드"로 결론 나서 화면 쪽은 이미 이 필드를 안 쓴다 — mock 응답·타입 선언에서는 아직 안 지웠다. `festivals.ts`의 host 요약 3곳, `search.ts`의 `hostType`도 같은 사정이라 실제로 지우려면 같이 봐야 한다(#64 조사 결과, 제거는 별도 이슈).
